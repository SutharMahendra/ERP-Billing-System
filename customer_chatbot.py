...
def chatbot_response(user_query):
        ...
        BUYER_LABELS = [
            ...
            "count", "list",
            # field search
            "find_phone","find_company","find_buyer"
            # filters
        ...
        if user_input == None or user_input == "":
            ...
            prediction = erp_chatbot.predict_label(user_input, BUYER_LABELS)
        ...
        if prediction == "count":
            total_buyer = len(df)
            response = f"Total Buyers: {total_buyer}"
        ...
        # ---- List Prediction ----
            ...
            if "company" in user_input:
                names = df["buyer_company_name"].dropna().astype(str).tolist()
                response = "🏢 Company Names:\n\n"
            else:
                names = df["buyer_name"].dropna().astype(str).tolist()
        ...
        # ---- find phone No Prediction ----
            ...
            if number:
                ...
                result = df[df["buyer_phone_no"].astype(str) == phone]

                if not result.empty:
                    company_name = result.iloc[0]["buyer_company_name"]
                    ...
                    response = f"❌ No buyer found with phone number {phone}"
            else:
        ...
        # ---- find company name Prediction ----
            ...
            company_list = df["buyer_company_name"].astype(str).tolist()
            ...
            if match:
                ...
                result = df[df["buyer_company_name"] == company_name]
                ...
                if not result.empty:
                    ...
                    for _, row in result.iterrows():
                        response += (
                            f"👤 Buyer: {row['buyer_name']}\n"
                            f"📞 Phone: {row['buyer_phone_no']}\n"
                            f"📧 Email: {row['buyer_email']}\n"
                            f"📍 City: {row['buyer_city']}\n"
                            f"🧾 GST: {row['buyer_gst_number']}\n"
                            "----------------------\n"
                        )
                else:
        ...
        # ---- find buyer Prediction ----
        elif prediction == "find_buyer":

           # Step 1: Clean user input (remove extra words)
           clean_input = re.sub(
               r"(find|search|get|buyer|details|of|is|available)",
               "",
               user_input.lower()
           ).strip()

           # Step 2: Get all buyer names
           buyer_list = df["buyer_name"].astype(str).tolist()

           # Step 3: Fuzzy match
           match = process.extractOne(clean_input, buyer_list)

           if match:
               buyer_name = match[0]

               # Step 4: Get matching data
               result = df[df["buyer_name"] == buyer_name]

               if not result.empty:
                   response = f"👤 Buyer Found: {buyer_name}\n\n"

                   for _, row in result.iterrows():
                       response += (
                           f"🏢 Company: {row['buyer_company_name']}\n"
                           f"📞 Phone: {row['buyer_phone_no']}\n"
                           f"📧 Email: {row['buyer_email']}\n"
                           f"📍 City: {row['buyer_city']}\n"
                           f"🧾 GST: {row['buyer_gst_number']}\n"
                   ...
                   response = f"❌ No data found for buyer {buyer_name}"
        ...
        # ---- Filter City name Prediction ----
            ...
            city_list = df["buyer_city"].astype(str).unique().tolist()

            # Step 2: Clean user input
            clean_input = re.sub(
                r"(show|list|buyers|from|in|who|are|the)",
                "",
                user_input.lower()
            ).strip()
            ...
            if match and match[1] > 70:
                ...
                # Step 4: Filter data
                result = df[df["buyer_city"] == city]

                if not result.empty:
                    response = f"📍 Buyers from {city}:\n\n"
                    ...
                    response = f"❌ No buyers found in {city}"
            else:
                response = "⚠️ City not recognized"
        
        #---- count buyer which dont have gst number ----
        elif prediction == "count_without_gst":
        
            # Filter rows where GST is empty or null
            result = df[
                df["buyer_gst_number"].isna() |
                (df["buyer_gst_number"].astype(str).str.strip() == "")
            ]
